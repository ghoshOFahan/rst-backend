...
import { getGame, getSocketRoom, setGame, setSocketRoom, GAME_SET_KEY, SOCKET_ROOM_KEY, getLastWord, pushWord, } from "./helpers/redis_helper.js";
...
io.on("connection", (socket) => {
    ...
    socket.on("reconnectRoom", async (oldsocketid) => {
        try {
            ...
            if (!roomId)
                return socket.emit("gameError", "could not find room while reconnecting due to roomID");
            const gameState = await getGame(redis, roomId);
            if (!gameState)
                return socket.emit("gameError", "could not find gameState while reconnecting due to gamestate");
            if (!gameState.players || !Array.isArray(gameState.players)) {
                return socket.emit("gameError", "players array not found in game state");
            }
            const playerIndex = gameState.players.findIndex((player) => player.id === oldsocketid);
            if (playerIndex === -1)
            ...
            socket.join(roomId);
            io.to(roomId).emit("gameStateUpdate", gameState);
            console.log("Player reconnected successfully:", gameState.players[playerIndex].username);
        }
        catch (error) {
    ...
    socket.on("disconnect", async () => {
        try {
            ...
            const cleanupTimeout = setTimeout(() => {
                (async () => {
                    try {
                        ...
                        if (!gameState)
                            return;
                        const updatedPlayers = gameState.players.filter((player) => player.id !== socket.id);
            ...
            console.log("player %s has %d grace period left in room%s", socket.id, GRACE_PERIOD, roomId);
        }
        catch (error) {
